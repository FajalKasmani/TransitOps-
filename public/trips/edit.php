        ...
        <form method="POST" action="edit.php?id=<?php echo $id; ?>" class="needs-validation" novalidate>
            <div class="row g-3">
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input type="text" class="form-control" id="source" name="source" required value="<?php echo htmlspecialchars($trip['source'], ENT_QUOTES, 'UTF-8'); ?>" <?php echo $isDriver ? 'disabled' : ''; ?> style="border-radius: 10px;">
                </div>

                <!-- Destination Location -->
                <div class="col-12 col-md-6">
                    <label for="destination" class="form-label fw-semibold text-muted small">Destination Location *</label>
                    <input type="text" class="form-control" id="destination" name="destination" required value="<?php echo htmlspecialchars($trip['destination'], ENT_QUOTES, 'UTF-8'); ?>" <?php echo $isDriver ? 'disabled' : ''; ?> style="border-radius: 10px;">
                </div>

                <!-- Cargo Weight -->
                <div class="col-12 col-md-4">
                    <label for="cargo_weight" class="form-label fw-semibold text-muted small">Cargo Weight (kg) *</label>
                    <input type="number" step="0.01" class="form-control" id="cargo_weight" name="cargo_weight" required value="<?php echo number_format((float)$trip['cargo_weight'], 2, '.', ''); ?>" <?php echo $isDriver ? 'disabled' : ''; ?> style="border-radius: 10px;">
                </div>

                <!-- Planned Distance -->
                <div class="col-12 col-md-4">
                    <label for="planned_distance" class="form-label fw-semibold text-muted small">Planned Distance (km) *</label>
                    <input type="number" step="0.01" class="form-control" id="planned_distance" name="planned_distance" required value="<?php echo number_format((float)$trip['planned_distance'], 2, '.', ''); ?>" <?php echo $isDriver ? 'disabled' : ''; ?> style="border-radius: 10px;">
                </div>

                <!-- Actual Distance (Editable when dispatched/completed) -->
                <div class="col-12 col-md-4">
                    <label for="actual_distance" class="form-label fw-semibold text-muted small">Actual Distance (km)</label>
                    <input type="number" step="0.01" class="form-control text-success fw-bold" id="actual_distance" name="actual_distance" value="<?php echo $trip['actual_distance'] !== null ? number_format((float)$trip['actual_distance'], 2, '.', '') : ''; ?>" style="border-radius: 10px;">
                </div>

                <!-- Dispatch Status (Editable by all allowed, but drivers only allow dispatched, completed, cancelled) -->
                <div class="col-12 col-md-4">
