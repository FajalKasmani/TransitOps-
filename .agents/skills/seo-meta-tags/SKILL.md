...
The [canonical-model.md](references/canonical-model.md) reference maps every abstract metadata concept to its concrete implementation in each framework, with constraints and validation rules. Use it to cross-check parity when adding or modifying fields.

The [tokens.json](references/tokens.json) registry is the single source of truth for all placeholder fields — register new tokens there first.

Run `scripts/validate.sh --check-templates` to verify template integrity against the registry.
...
When implementing metadata with user-supplied or database-sourced values, follow these rules:

### Input Sanitization

| Context | Risk | Mitigation |
| --- | --- | --- |
| HTML meta tag `content` attributes | XSS via attribute injection | Escape `"`, `<`, `>`, `&` in all attribute values |
| JSON-LD string fields | Script injection via malformed JSON | Use `JSON.stringify()` for all values — never concatenate raw strings into JSON-LD |
| `<script type="application/ld+json">` blocks | Code execution if user input contains `</script>` | Escape `</` sequences in JSON-LD output |
| Dynamic HTML content (blog posts, CMS data) | XSS via raw HTML rendering | Never use `dangerouslySetInnerHTML` or `v-html` without sanitizing first (use [DOMPurify](https://github.com/cure53/DOMPurify) or equivalent) |
| URL fields (canonical, OG url, image URLs) | Open redirect, SSRF | Validate URLs start with `https://` and belong to expected domains |

### Rules for Agents

- When generating metadata from user-provided data (e.g., CMS content, API responses, route parameters), **always escape HTML entities** in attribute values before inserting into templates.
- When generating JSON-LD from dynamic data, **always use `JSON.stringify()`** to serialize values — never build JSON by string concatenation.
- When the user's code renders fetched HTML content, **always recommend a sanitization library** (DOMPurify for browsers, sanitize-html for Node.js). Never output `dangerouslySetInnerHTML` or `v-html` without sanitization.
- Treat all `{{PLACEHOLDER}}` tokens as potentially attacker-controlled when the values come from external sources. Validate and sanitize before substitution.
- **External dependencies**: When installing packages (`react-helmet-async`, `@unhead/vue`, `vite-plugin-html`), verify the package name and version on [npmjs.com](https://www.npmjs.com/) before running `npm install`. Pin versions in production.
