...
```bash
npm install vite-plugin-html@latest -D  # verify version at https://www.npmjs.com/package/vite-plugin-html
```
...
```bash
npm install react-helmet-async@latest  # verify version at https://www.npmjs.com/package/react-helmet-async
```
...
type SEOProps = {
  ...
  twitterCreatorHandle?: string  // fallback: twitterHandle
  canonical?: string      // falls back to url if not provided
} & (CommonSEOType | ArticleSEOType)
...
type ArticleSEOType = {
  type: 'article'
  publishedTime: string   // required when type='article'
  modifiedTime?: string
}
```
...
function BlogPost() {
  ...
  return (
    <>
      ...
      <article>
        <h1>{post.title}</h1>
        {/* Render post content safely. If content is HTML, sanitize it
...
```bash
npm install @unhead/vue@latest  # verify version at https://www.npmjs.com/package/@unhead/vue
```
...
Same discriminated union interface as React, using Vue `withDefaults`:
...
<template>
  <div v-if="post">
    ...
    <article>
      <h1>{{ post.title }}</h1>
      <!-- If content is HTML, sanitize before rendering:
           import DOMPurify from 'dompurify'
